...
def find_max_steal(wealth):
    ...
    return find_max_steal_recurse(wealth, 0, dp)

def find_max_steal_recurse(wealth, currentIndex, dp):
    if currentIndex >= len(wealth):
        return 0
    if dp[currentIndex] == 0:
        stealCurrent = wealth[currentIndex] + find_max_steal_recurse(wealth, currentIndex + 2, dp)
        skipCurrent = find_max_steal_recurse(wealth, currentIndex + 1, dp)
